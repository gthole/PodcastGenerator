...
if(isUserLogged()) {
    ...
    if (isset($_GET['action']) AND $_GET['action']=="change") { // if action is set
        ...
        $gaId = $_POST['gaId'];
        if ($gaId != "") {
            $gaId = stripslashes($gaId);
            $gaId = strip_tags($gaId);
            $gaId = htmlspecialchars($gaId);
            $gaId = depurateContent($gaId);
            $podcast_ga_id = $gaId;
        }else{
            $PG_mainbody .= '<p>'._("Google Analytics ID").' '._("is empty... (it will be ignored)").'</p>';
        }
